...
class ActiveDirectory {
    ...
    public function __construct() {
        ...
        if (!LDAP_ENABLED) {
            $this->enabled = false;
            error_log('LDAP extension is not enabled. Please enable php_ldap in php.ini');
            return;
        }
...
/**
 * Check if LDAP extension is available and provide instructions if not
 */
function checkLDAPExtension() {
    if (!LDAP_ENABLED) {
        return [
            'enabled' => false,
            'message' => 'LDAP extension is not enabled.',
            'instructions' => [
                'windows_xampp' => [
